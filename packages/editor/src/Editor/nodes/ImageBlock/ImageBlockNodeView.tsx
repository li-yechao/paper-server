...
import { NodeViewReact, StrictProsemirrorNode } from '../../lib/Node'
...
export default class ImageBlockNodeView extends NodeViewReact<ImageBlockAttrs> {
  ...
  override contentDOM = document.createElement('figcaption')
  ...
  component = () => {
    ...
    useEffect(() => {
      if (state.current.visible) {
        ;(async () => {
          const src = (this.attrs.src && (await this.options.source(this.attrs.src))) ?? undefined
          setState({ src })
        })()
      }
